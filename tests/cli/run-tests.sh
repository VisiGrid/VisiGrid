...
run_test() {
    ...
    # Check exported CSV files (expected.FOO.csv vs actual_FOO.csv)
    for expected_csv in "$test_dir"/expected.*.csv; do
        if [[ -f "$expected_csv" ]]; then
            local basename=$(basename "$expected_csv")
            # expected.unmatched.csv -> actual_unmatched.csv
            local suffix="${basename#expected.}"
            local actual_csv="$test_dir/actual_${suffix}"
            if [[ ! -f "$actual_csv" ]]; then
                echo -e "${RED}FAIL${NC} $test_name: expected export file not written: $actual_csv"
                failed=1
            elif ! diff -q "$expected_csv" "$actual_csv" > /dev/null 2>&1; then
                echo -e "${RED}FAIL${NC} $test_name: CSV mismatch ($suffix)"
                echo "  Expected:"
                head -5 "$expected_csv" | sed 's/^/    /'
                echo "  Actual:"
                head -5 "$actual_csv" | sed 's/^/    /'
                failed=1
            fi
        fi
    done
    ...
    # Clean up any actual_*.csv files generated by --export
    rm -f "$test_dir"/actual_*.csv
