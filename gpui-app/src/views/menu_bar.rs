...
fn render_file_menu(highlight: Option<usize>, text_primary: Hsla, text_muted: Hsla, selection_bg: Hsla, border: Hsla, cx: &mut Context<Spreadsheet>) -> Div {
    ...
    div()
        ...
        .child(menu_item("Open Cloud...", None, 4, h(4), text_primary, text_muted, selection_bg, cx, |this, _window, cx| { this.close_menu(cx); this.cloud_open(cx); }))
        .child(menu_item("Move to Cloud...", None, 5, h(5), text_primary, text_muted, selection_bg, cx, |this, _window, cx| { this.close_menu(cx); this.cloud_move_to_cloud(cx); }))
        .child(menu_separator(border))
