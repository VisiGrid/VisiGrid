...
impl Spreadsheet {
    fn finish_workbook_snapshot_restore(
        ...
        let active_sheet = self.sheet_index(cx);
        self.active_view_state_mut().active_sheet = active_sheet;
        self.bump_cells_rev();
    }
        ...
        if let Some(entry) = self.history.undo() {
            match entry.action {
                ...
                UndoAction::NamedRangeCreated { named_range } => {
                    ...
                    self.workbook.update(cx, |wb, _| { wb.delete_named_range(name); });
                    self.bump_cells_rev();
                    self.status_message = Some(format!("Undo: removed '{}'", name));
                }
                UndoAction::NamedRangeRenamed { old_name, new_name } => {
                    // Rename back to original name
                    self.workbook.update(cx, |wb, _| { let _ = wb.rename_named_range(&new_name, &old_name); });
                    self.bump_cells_rev();
                ...
                UndoAction::NamedRangeDescriptionChanged { name, old_description, .. } => {
                    ...
                    self.workbook.update(cx, |wb, _| { let _ = wb.named_ranges_mut().set_description(&name, old_description.clone()); });
                ...
                UndoAction::Group { actions, description } => {
                    ...
                    for action in actions.into_iter().rev() {
                        self.apply_undo_action(action, cx);
                    }
                    self.status_message = Some(format!("Undo: {}", description));
                }
    ...
    fn apply_undo_action(&mut self, action: UndoAction, cx: &mut Context<Self>) {
        match action {
            ...
            UndoAction::NamedRangeCreated { named_range } => {
                self.workbook.update(cx, |wb, _| { wb.delete_named_range(&named_range.name); });
                self.bump_cells_rev();
            }
            UndoAction::NamedRangeRenamed { old_name, new_name } => {
                self.workbook.update(cx, |wb, _| { let _ = wb.rename_named_range(&new_name, &old_name); });
    ...
    fn apply_redo_action(&mut self, action: UndoAction, cx: &mut Context<Self>) {
        match action {
            ...
            UndoAction::NamedRangeRenamed { old_name, new_name } => {
                self.wb_mut(cx, |wb| { let _ = wb.rename_named_range(&old_name, &new_name); });
                self.bump_cells_rev();
            }
            UndoAction::NamedRangeDescriptionChanged { name, new_description, .. } => {
                self.wb_mut(cx, |wb| { let _ = wb.named_ranges_mut().set_description(&name, new_description.clone()); });
            }
            UndoAction::Group { actions, .. } => {
        ...
        if let Some(entry) = self.history.redo() {
            match entry.action {
                ...
                UndoAction::NamedRangeCreated { ref named_range } => {
                    ...
                    let name = named_range.name.clone();
                    self.workbook.update(cx, |wb, _| { let _ = wb.named_ranges_mut().set(named_range.clone()); });
                    self.bump_cells_rev();
                    self.status_message = Some(format!("Redo: recreated '{}'", name));
                }
                UndoAction::NamedRangeRenamed { old_name, new_name } => {
                    // Rename again to new name
                    self.workbook.update(cx, |wb, _| { let _ = wb.rename_named_range(&old_name, &new_name); });
                    self.bump_cells_rev();
                    self.status_message = Some(format!("Redo: renamed to '{}'", new_name));
                }
                UndoAction::NamedRangeDescriptionChanged { name, new_description, .. } => {
                    // Apply the new description
                    self.workbook.update(cx, |wb, _| { let _ = wb.named_ranges_mut().set_description(&name, new_description.clone()); });
    ...
    fn revalidate_range(&mut self, range: &visigrid_engine::validation::CellRange, cx: &mut Context<Self>) {
