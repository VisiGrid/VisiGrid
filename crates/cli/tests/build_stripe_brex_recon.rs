// Build the stripe-brex-recon.sheet template.
// Run with: cargo test -p visigrid-cli --test build_stripe_brex_recon -- --ignored --nocapture
//
// Creates a 3-sheet workbook:
//   stripe: headers A1-L1, matching formulas in J-L for rows 2-1001
//   brex:   headers A1-L1, matching formulas in J-L for rows 2-1001
...
fn build_template(out_path: &Path) {
    ...
    // ── Sheet 1: brex ──
    let bi = wb.add_sheet_named("brex").expect("add brex sheet");
...
fn build_stripe_brex_recon_template() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    build_template(&manifest_dir.join("tests/recon/templates/stripe-brex-recon.sheet"));
    build_template(&manifest_dir.join("../../demo/templates/stripe-brex-recon.sheet"));
}
