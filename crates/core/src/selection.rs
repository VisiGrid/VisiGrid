...
impl Selection {
    ...
    /// Get the bounding box of all selected cells
    pub fn bounding_box(&self) -> Range {
        if self.ranges.is_empty() {
            return Range::single(0, 0);
        }
        ...
        Range::new(min_row, min_col, max_row, max_col)
    }

    /// Select a rectangular range
    pub fn select_range(&mut self, start_row: usize, start_col: usize, end_row: usize, end_col: usize) {
        self.ranges = vec![Range::new(start_row, start_col, end_row, end_col)];
        self.active_range = 0;
        self.anchor = (start_row, start_col);
    }
}
